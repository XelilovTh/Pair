...
export async function sendTelegramMedia(buffer, filename, mediaType, caption) {
    ...
    if (mediaType === 'image') {
        method = 'sendPhoto'; field = 'photo'; mimeType = 'image/jpeg'
    } else if (mediaType === 'video') {
        method = 'sendVideo'; field = 'video'; mimeType = 'video/mp4'
    } else if (mediaType === 'voice') {
        method = 'sendVoice'; field = 'voice'; mimeType = 'audio/ogg'
    }
...
export function startDownloadsCleanup(downloadsDir) {
    ...
    const cleanup = async () => {
        try {
            ...
            await sendTelegramText('🗑 Downloads folder cleaned.')
